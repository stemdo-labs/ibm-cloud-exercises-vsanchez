...
resource "ibm_is_ssh_key" "ssh_key" {
  name       = "ssh-key-valentino-ej07"
  public_key = var.public_key
  resource_group = var.resource_group
}


resource "ibm_is_instance" "vm_valentino_ej07_1" {
  name              = "vm-valentino-ej07-1"
  vpc               = ibm_is_vpc.vpc.id
  profile           = "bx2-2x8"
  zone              = "eu-es-1"
...
resource "ibm_is_instance" "vm_valentino_ej07_2" {
  name              = "vm-valentino-ej07-2"
  vpc               = ibm_is_vpc.vpc.id
  profile           = "bx2-2x8"
  zone              = "eu-es-2"
  keys = [ibm_is_ssh_key.ssh_key.id]  
...
resource "ibm_is_floating_ip" "ip_public_valentino_ej07_1" {
  name   = "ip-public-valentino-ej07-1"
  target = ibm_is_instance.vm_valentino_ej07_1.primary_network_interface.0.id
  resource_group = var.resource_group
}

resource "ibm_is_floating_ip" "ip_public_valentino_ej07_2" {
  name   = "ip-public-valentino-ej07-2"
  target = ibm_is_instance.vm_valentino_ej07_2.primary_network_interface.0.id
  resource_group = var.resource_group
}

resource "ibm_is_lb" "load_balancer" {
  resource_group = var.resource_group
  name    = "load-balancer-valentino-ej07"
...
resource "ibm_is_lb_pool_member" "member1" {
  ...
  port     = 80
  target_address = ibm_is_instance.vm_valentino_ej07_1.primary_network_interface[0].primary_ipv4_address
}
...
resource "ibm_is_lb_pool_member" "member2" {
  ...
  pool     = ibm_is_lb_pool.lb_pool.id
  port     = 80
  target_address = ibm_is_instance.vm_valentino_ej07_2.primary_network_interface[0].primary_ipv4_address
}
...
resource "ibm_resource_instance" "cos_instance" {
  name              = "cos-instance-valentino-ej07"
...
resource "ibm_cos_bucket" "bucket" {
  bucket_name          = "bucket-valentino-ej07"
